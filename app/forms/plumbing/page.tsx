...
const PlumbingSchema = z.object({
  ...
  fixtures: z.coerce.number().int().nonnegative("Must be 0 or more"),
  contractorName: z.string().min(1, "Contractor name is required"),
  contractorLicense: z.string().min(1, "License number is required"),
});

type PlumbingFormData = z.infer<typeof PlumbingSchema>;

export default function PlumbingFormPage() {
  ...
  } = useForm<PlumbingFormData>({ resolver: zodResolver(PlumbingSchema) });
  ...
  const onSubmit = async (data: PlumbingFormData) => {
    setStatus("idle");
    try {
      const res = await fetch("/api/forms", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ type: "plumbing", ...data }),
      });
  ...
  return (
    <FormLayout
      ...
      titleIcon={<Droplets className="h-6 w-6" />}
      description={"Complete the form to submit your plumbing permit application."}
      onSubmit={handleSubmit(onSubmit)}
      successMessage={status === "success" ? "Successfully submitted Plumbing Permit Application." : undefined}
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="md:col-span-2">
          ...
          <textarea rows={4} {...register("descriptionOfWork")} className="mt-1 w-full rounded-md border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
          {errors.descriptionOfWork && <p className="text-sm text-red-600 mt-1">{errors.descriptionOfWork.message}</p>}
        </div>
        <div>
          <label className="block text-sm font-medium">Number of Fixtures</label>
          <input type="number" {...register("fixtures", { valueAsNumber: true })} className="mt-1 w-full rounded-md border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
          {errors.fixtures && <p className="text-sm text-red-600 mt-1">{errors.fixtures.message}</p>}
        </div>
        <div>
