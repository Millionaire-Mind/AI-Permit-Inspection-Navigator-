...
const PlumbingSchema = z.object({
  ...
  numberOfFixtures: z.coerce.number().int().nonnegative("Must be 0 or more"),
  ...
  return (
    <div className="p-6 space-y-4">
      <h1 className="text-2xl font-bold flex items-center gap-2">
        <Droplets className="h-6 w-6" /> Plumbing Permit Application
      </h1>
      {status === "success" && (
        <div className="rounded-md bg-green-50 text-green-800 px-4 py-3 ring-1 ring-green-200">
          Successfully submitted Plumbing Permit Application.
        </div>
      )}
      {status === "error" && (
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="number" {...register("numberOfFixtures", { valueAsNumber: true })} className="mt-1 w-full rounded-md border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                {errors.numberOfFixtures && <p className="text-sm text-red-600 mt-1">{errors.numberOfFixtures.message}</p>}
              </div>
              <div>
